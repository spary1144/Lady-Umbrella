...
USecondOrder2DSystem::USecondOrder2DSystem()
{
...
USecondOrder2DSystem::USecondOrder2DSystem(const float _Frequency, const float _Dampening, const float _InitialResponse)
{
	// Set this component to be initialized when the game starts, and to be ticked every frame.  You can turn these features
	// off to improve performance if you don't need them.
	PrimaryComponentTick.bCanEverTick = false;
...
void USecondOrder2DSystem::BeginPlay()
{
	Super::BeginPlay();
...
// Called every frame
void USecondOrder2DSystem::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
...
FVector2D USecondOrder2DSystem::Update(const FVector2D& _TargetValue, const float _DeltaTime, const FVector2D& _InputVelocity)
{
...
FVector2D USecondOrder2DSystem::UpdateWithEstimatedVelocity(const FVector2D& _TargetValue, const float _DeltaTime)
{
	// Checking that the DeltaTime is not 0.
	if (_DeltaTime <= 0)
	{
	...
	FVector2D inputVelocity = (_TargetValue - StoredUnsmoothedTargetValue) / _DeltaTime;

	// Calling the general function.
	return Update(_TargetValue, _DeltaTime, inputVelocity);
}

void USecondOrder2DSystem::ChangeConstants(const float _Frequency, const float _Dampening, const float _InitialResponse)
{
...
void USecondOrder2DSystem::ResetSystem()
{
	LastSmoothedValue = StoredUnsmoothedTargetValue;
	LastSmoothedSpeed = FVector2D::Zero();
}

void USecondOrder2DSystem::ResetSystemToValue(const FVector2D& _InitValue)
{
...
void USecondOrder2DSystem::SetSystemStoredSpeed(const FVector2D& _NewSpeed)
{
	LastSmoothedSpeed = _NewSpeed;
}

void USecondOrder2DSystem::SetSystemStoredValue(const FVector2D& _NewValue)
{
