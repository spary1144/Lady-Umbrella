...
#include "SecondOrder3DSystem.generated.h"


UCLASS( ClassGroup=(Custom), meta=(BlueprintSpawnableComponent) )
class LADYUMBRELLA_API USecondOrder3DSystem : public UActorComponent
{
	GENERATED_BODY()

public:	
	// Sets default values for this component's properties
	USecondOrder3DSystem();
 	...
 	*/
	USecondOrder3DSystem(const float _Frequency, const float _Dampening, const float _InitialResponse);
...
public:	
	...
	FVector Update(const FVector& _TargetValue, const float _DeltaTime, const FVector& _InputVelocity);
 	...
 	*/
	UFUNCTION(BlueprintCallable)
	FVector UpdateWithEstimatedVelocity(const FVector& _TargetValue, const float _DeltaTime);
	...
	*/
	...
	UFUNCTION(BlueprintCallable)
	void ResetSystemToValue(const FVector& _InitValue);

	UFUNCTION(BlueprintCallable)
	void SetSystemStoredSpeed(const FVector& _NewSpeed);

	UFUNCTION(BlueprintCallable)
	void SetSystemStoredValue(const FVector& _NewValue);

	UFUNCTION(BlueprintCallable)
	FORCEINLINE FVector GetSystemStoredSpeed() const{return LastSmoothedSpeed;};
	
	UFUNCTION(BlueprintCallable)
	FORCEINLINE FVector GetSystemStoredValue() const{return LastSmoothedValue;};
	
private:
	UPROPERTY()
	FVector StoredUnsmoothedTargetValue = FVector::Zero();
	UPROPERTY()
	FVector LastSmoothedValue = FVector::Zero();
	UPROPERTY()
	FVector LastSmoothedSpeed = FVector::Zero();
